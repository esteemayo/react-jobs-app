...
export const fetchSingleJobBySlugAsync = createAsyncThunk('jobs/fetchSingleJobBySlug', async (slug) => {
    const { data: { job } } = await jobService.getJobBySlug(slug);
    return job;
});

export const createJobAsync = createAsyncThunk('jobs/createJob', async (job) => {
    const { data: { job: newJob } } = await jobService.createJob(job);
    return newJob;
});

export const updateJobAsync = createAsyncThunk('jobs/updateJob', async (jobId, job) => {
